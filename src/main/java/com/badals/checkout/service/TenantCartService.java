...
import com.badals.checkout.domain.TenantOrderItem;
...
public class TenantCartService {
    ...
    @Autowired
    TenantCartRepository cartRepository;
    ...
    @Autowired
    private TenantOrderRepository orderRepository;
   ...
   public String calculateValue(CartDTO cart) {
    ...
    public BigDecimal calculateTotal(TenantCart cart) {
        BigDecimal sum = BigDecimal.valueOf(cart.getItems().stream().mapToDouble(x -> x.getPrice().doubleValue() * x.getQuantity().doubleValue()).sum());
        sum = sum.add(carrierService.getCarrierCost(cart.getCarrier()));
        return sum;
    }

    public BigDecimal calculateSubtotal(TenantCart cart) {
        BigDecimal sum = BigDecimal.valueOf(cart.getItems().stream().mapToDouble(x -> x.getPrice().doubleValue() * x.getQuantity().doubleValue()).sum());
        return sum;
    }

    public void setPaymentToken(Long cartId, String paymentRef, String paymentToken) {
        TenantCart cart = cartRepository.findById(cartId).get();
    ...
    @Transactional
    public TenantOrder createOrder(TenantCart cart, String paymentMethod, boolean isPaid) {
    ...
    public TenantOrder createOrderWithPaymentByPaymentToken(String paymentKey) throws InvalidCartException {
        TenantCart cart = cartRepository.findByPaymentTokenAndCheckedOut(paymentKey, false).orElse(null);
        if(cart == null)
        ...
        TenantOrder order = createOrder(cart, cart.getPayment(), p.prePay);
        if(p.prePay) {
    ...
    public CartDTO findBySecureKeyWithLock(String secureKey) throws InvalidCartException, LockedCartException {
       ...
       // log.info("Entity {}", cartRepository.findBySecureKey(secureKey));
        ...
        TenantCart cart = cartRepository.findBySecureKey(secureKey).orElseThrow(() -> new InvalidCartException("Cart Not Found"));
        if(cart.getLock())
    ...
    public OrderConfirmationResponse createOrder(String paymentKey) throws InvalidCartException {
        TenantOrder o = createOrderWithPaymentByPaymentToken(paymentKey);
